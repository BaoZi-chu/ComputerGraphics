...
CVector3 operator *(const CVector3& v, float k)//矢量与常量的积
{
	CVector3 vector;
	vector.x = v.x * k;
	vector.y = v.y * k;
...
CVector3 operator *(float k, const CVector3& v)//常量与矢量的积
{
	CVector3 vector;
	vector.x = v.x * k;
	vector.y = v.y * k;
...
CVector3 operator /(const CVector3& v, float k)//矢量数除
{
	if (fabs(k) < 1e-6)
		k = 1.0;
	CVector3 vector;
...
float Dot(const CVector3& v0, const CVector3& v1)//矢量的点积
{
